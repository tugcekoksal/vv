          ...
          child: Column(
            ...
            children: [
              ...
              Wrap(
                ...
                : mapBikeController.availableFiltersList.map((filterLabel) => 
                  Obx(
                    () {
                      return BuildButtonFilter(
                          label: filterLabel, 
                          setFilters: mapBikeController.setFilters, 
                          isSelected: mapBikeController.selectedFiltersList.contains(filterLabel), 
              ...
              Wrap(
                spacing: 8.0,
                direction: Axis.horizontal,
                children: mapBikeController.availableStatus.map((statusLabel) => 
                  Obx(
                    () {
                      return BuildButtonStatus(
                          label: statusLabel, 
                          setFilters: mapBikeController.setStatus, 
                          isSelected: mapBikeController.selectedStatus.contains(statusLabel), 
                          screenWidth: screenWidth
                      );
                    }
                  )
...
class BuildButtonFilter extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return FilterChip(
      ...
      shape: StadiumBorder(
        side: BorderSide(
          color: isSelected ? GlobalStyles.blue : GlobalStyles.backgroundLightGrey,
          width: 1.5
        )
      ),
  ...
  Widget build(BuildContext context) {
    return FilterChip(
      label: Text(
        label,
        style: TextStyle(
          color: isSelected ? colorStatusTypology[label] : GlobalStyles.greyLine,
          fontSize: 12.0,
          fontWeight: FontWeight.w600
        )
      ),
      ...
      shape: StadiumBorder(
        side: BorderSide(
          color: isSelected ? colorStatusTypology[label] : GlobalStyles.backgroundLightGrey,
          width: 1.5
        )
      ),
      onSelected: (bool value) => setFilters(value, label),
